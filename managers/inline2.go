...
func NewInline2Manager(logger *slog.Logger, config *core.Config, managerConfig *core.Manager) IManager2 {
...
func (manager *Inline2Manager) ExtractDependencies(filePath string) ([]*core.Dependency, error) {
	...
	for _, match := range matchesIndex {
		...
		if packageOk {
			newDepencency.PackageName = packageObject[0].Value
		} else if inlineConfig.PackageName != "" {
			newDepencency.PackageName = inlineConfig.PackageName
		}
		if versioningOk {
