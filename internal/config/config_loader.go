...
func (c loader) HasProjectConfig() (bool, error) {
...
func (c loader) loadConfigFromFile(parentInfo, newInfo *configInfo) (*RootConfig, error) {
	...
	if exists, err := core.FileExists(newInfo.Location); err != nil {
		return nil, err
	} else if exists {
		return c.readConfigFromFile(newInfo.Location)
	}
	...
	if parentInfo != nil && parentInfo.Type == infoTypeFile && parentInfo.Location != "" {
		...
		if exists, err := core.FileExists(tempPresetPath); err != nil {
			return nil, err
		} else if exists {
